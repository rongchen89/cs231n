...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    ############################################################################
    ### forward pass
    # init_hidden: shape(N, H)
    init_hidden = features.dot(W_proj) + b_proj
    ...
    loss, dloss = temporal_softmax_loss(affine_out, captions_out, mask)
    ### backward pass
    drnn_h, dW_vocab, db_vocab = temporal_affine_backward(dloss, affine_cache)
    ...
    dW_embed = word_embedding_backward(dword_embedding_out, word_embedding_cache)
    grads['W_embed'] = dW_embed

    grads['b_proj'] = dinit_hidden.sum(axis=0)
    grads['W_proj'] = features.T.dot(dinit_hidden)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def sample(self, features, max_length=30):
    ...
    # a loop.                                                                 #
    ###########################################################################
    # initialize the hidden state from image features
    init_hidden = features.dot(W_proj) + b_proj
    _, D = W_embed.shape
    ...
    for t in xrange(max_length):
        ...
        # step 4
        prev_words = affine_out.argmax(axis=1)
